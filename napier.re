...
ロフト機能を使います。作りたいフィーチャの両端をスケッチで作って指定してやれば、自動的につないでくれます。
起点のスケッチを(本来の)立方体の頂点に相当する部分に描いておき、
飛び出した棒の側面に沿って終点のスケッチを描けば、ピッタリくっつく三本の棒が出来上がります。
...
闇魔術を使って各辺をバラバラにして並べなおしました。こうすると@<strong>{2,468円}で済みます。
差額の7,813円がパーツ分割の苦労、あるいは一発成型の名人(名出力機)芸の価値に相当するわけですね。
