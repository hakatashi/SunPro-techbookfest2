...
トランジスタの代わりにリレーを使ってコンピューターを作るというのは、なにも夢物語ではありません。
今では忘れ去られてしまいましたが、地球ではリレーを使ったコンピューターが多く作られてきました。

=== Zuse Z3

そもそも、「世界初のコンピューター」の一つ@<fn>{the-first-computer}である@<kw>{Zuse Z3}は、リレー式のコンピューターでした。
...
//footnote[z3-turing-complete][普通にと言うのは、実はZ3の完成から50年以上経った1998年に、特殊な方法でZ3でチューリングマシンを実装する方法が示され、チューリング完全であることが証明されてしまったためです。しかしいずれにせよZ3でプログラミングするのは厳しいので、チューリング不完全として扱うことにします。]
...
その中でも特に注目したいのは、「チューリング完全な世界最小のリレー式コンピューター」を自称する@<kw>{TIM-8}@<bib>{tim-8}です。

TIM-8は@<kw>{Rory Mangles}らによって制作された8ビットの@<kw>{ハーバード・アーキテクチャ}のコンピューターであり、
8つのレジスタと8つの命令セット、そしてディップスイッチで実装された16バイトのROMを持っています。
これは現代のコンピューターとほぼ同じ構成で、条件分岐やループといった命令も有しています。
